...
class NearbySchoolsScreen extends ConsumerStatefulWidget {
  const NearbySchoolsScreen({super.key});

  @override
  ConsumerState<NearbySchoolsScreen> createState() =>
      _NearbySchoolsScreenState();
}

class _NearbySchoolsScreenState extends ConsumerState<NearbySchoolsScreen> {
  ...
  Future<void> _load({
    ...
    try {
      ...
      final (schools, radiusUsed) =
          await NearbyPlacesService.searchNearbySchools(
        resolvedPosition,
  ...
  List<NearbySchool> get _visibleSchools {
    var list = _allSchools;
    if (_selectedLevel != 'All') {
      list = list.where((s) => s.level == _selectedLevel).toList();
    }
    if (_savedOnly) {
      list = list.where((s) => _savedIds.contains(s.osmId)).toList();
    }
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        const Spacer(),
                        Text(
                          '${_visibleSchools.length} found${_radiusExpandedLabel()}',
                          style: const TextStyle(
    ...
    switch (_screenState) {
        ...
        return ExploreMessageView(
          ...
          message: _permissionDeniedForever
              ? 'Location permission is blocked. EduNavigate AI needs it to '
                  'find schools near where you are right now. Enable it in '
                  'Settings, then try again.'
              : 'EduNavigate AI needs your location to search for schools '
      ...
      case _ScreenState.serviceDisabled:
        return ExploreMessageView(
          ...
          message:
              ...
              'schools near your current location.',
      ...
      case _ScreenState.noInternet:
        return ExploreMessageView(
          ...
          message:
              'Searching nearby schools needs an internet connection. Check '
      ...
      case _ScreenState.apiError:
        return ExploreMessageView(
          ...
          title: 'Couldn\'t load nearby schools',
      ...
      case _ScreenState.ready:
        ...
        if (schools.isEmpty) {
          return ExploreMessageView(
            ...
            title: 'No nearby institutions found',
            message: 'We couldn\'t find any schools, colleges, or universities '
                'nearby. Try searching a larger area.',
        ...
        return RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) => _SchoolCard(
              school: schools[i],
              isSaved: _savedIds.contains(schools[i].osmId),
              onToggleSaved: () => _toggleSaved(schools[i].osmId),
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute<void>(
                  builder: (_) => SchoolDetailScreen(school: schools[i]),
                ),
              ),
            ),
          ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    ...
                    Expanded(
                      child: Text(
                        school.address.isEmpty
                            ? school.distanceLabel
                            : '${school.distanceLabel} · ${school.address}',
                        style: const TextStyle(
                            color: Colors.white38, fontSize: 12),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
